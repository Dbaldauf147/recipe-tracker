// Proxies a date range of finalized Rally events into Prep Day's Week Plan.
// The call to Rally happens server-side so the shared secret (RALLY_EXPORT_KEY)
// never reaches the browser and there's no cross-origin request. Returns
// { eventsByDay: { 'YYYY-MM-DD': [{ id, title, location }] } }.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const r = await fetch(`${RALLY_BASE_URL}/api/plans-export?${params}`);
