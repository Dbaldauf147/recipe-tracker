...
export function renderPrepareWeek(log, startDateKey) {
  const days = [];
  for (let i = 0; i < 7; i++) {
  ...
  function macrosRowHtml() {
    const cells = days.map(d => {
      ...
      return `<td style="${cellBase}color:#111827;font-size:11px;text-align:center;">`
        + `<div>Cal <strong>${Math.round(m.calories)}</strong></div>`
        + `<div>P <strong>${Math.round(m.protein)}g</strong></div>`
        + `<div>C <strong>${Math.round(m.carbs)}g</strong></div>`
        + `<div>F <strong>${Math.round(m.fat)}g</strong></div>`
        + `</td>`;
    }).join('');
  ...
  for (const d of days) {
    ...
    if (d.macros) {
      const m = d.macros;
      textParts.push(`  Macros: ${Math.round(m.calories)} cal / ${Math.round(m.protein)}p / ${Math.round(m.carbs)}c / ${Math.round(m.fat)}f`);
    }
    textParts.push('');
  }
...
export function renderMealReminder({ remaining, log, dateKey }) {
  ...
  const week = renderPrepareWeek(log, dateKey);
  ...
  const text = [intro, '', planHeader, subHeader, '', week.text, footer, '', '— Prep Day'].join('\n');
