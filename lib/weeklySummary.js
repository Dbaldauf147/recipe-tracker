...
export function summarizeGoals(data, week, config = {}) {
  ...
  const saunaTarget = numOr(config.saunaGoal, 0);
...
function summarizeProgress(data, week) {
  ...
  // name → muscle group, and name → the user's own discipline tag. Library rows
  // win over custom entries, matching exerciseTypeForName in both apps.
...
export function summarizeWeek(data, week, { withProgress = false, goalsConfig = null } = {}) {
  return {
    ...
    weekGoals: goalsConfig ? summarizeGoals(data, week, goalsConfig) : null,
...
function fmtGoalValue(g) {
  ...
  return `${fmtNum(g.actual, Number.isInteger(g.actual) ? 0 : 1)}${unit}`
    + ` / ${fmtNum(g.target, Number.isInteger(g.target) ? 0 : 1)}${unit}${suffix}`;
}
...
function renderGoalsTable(weekGoals) {
  const rows = weekGoals.rows.map(g => {
    ...
    return `<tr>`
      ...
      + `<td style="${cell}width:38%;">${goalBar(pct, g.met)}</td>`
      + `<td style="${cell}white-space:nowrap;text-align:right;color:#111827;font-variant-numeric:tabular-nums;">${escapeHtml(fmtGoalValue(g))}</td>`
      + `<td style="${cell}white-space:nowrap;text-align:right;">`
      + (g.met
        ? `<span style="color:#16a34a;font-weight:700;">✓ met</span>`
  ...
  return `<p style="font-size:13px;margin:0 0 8px 0;color:#374151;">`
    ...
    + `<thead><tr>${head('Goal')}${head('Progress')}${head('Result', 'right')}${head('', 'right')}</tr></thead>`
...
function goalsTableText(weekGoals) {
  ...
  for (const g of weekGoals.rows) {
    ...
    out.push(`  ${g.label.padEnd(width)}  ${'█'.repeat(bars)}${'░'.repeat(10 - bars)}  `
      + `${fmtGoalValue(g)}  ${g.met ? '✓ met' : goalShortfall(g)}`);
  }
  return out;
}
...
export function renderWeeklySummary({ stats, priorStats, goals = null, bodyStats = null, weightUnit = 'lb', name = '' }) {
  ...
  if (prog && prog.analysed) {
    const line = (r, color) => {
      const grp = r.group ? ` <span style="color:#9ca3af;">(${escapeHtml(r.group)})</span>` : '';
      const pct = r.deltaPct == null ? '' : ` <span style="color:#9ca3af;">${signedPct(r.deltaPct)} est. 1RM</span>`;
      const vol = r.annotations.includes('volume-down') ? ` <span style="color:#dc2626;">· volume down</span>` : '';
      const sess = ` <span style="color:#9ca3af;">· ${r.sessions} session${r.sessions === 1 ? '' : 's'}</span>`;
      return `<span style="color:${color};">${escapeHtml(r.name)}</span>${grp}${pct}${vol}${sess}`;
    };
    if (prog.decreasing.length > 0) {
      attentionRows.push(row(
        `Decreasing (${prog.decreasing.length})`,
        prog.decreasing.map(r => line(r, '#dc2626')).join('<br/>'),
      ));
    }
    if (prog.stagnating.length > 0) {
      attentionRows.push(row(
        `Stagnating (${prog.stagnating.length})`,
        prog.stagnating.map(r => line(r, '#d4a017')).join('<br/>'),
      ));
    }
    if (attentionRows.length === 0) {
      attentionRows.push(row('Trends', 'Nothing decreasing or stagnating — everything with a baseline is holding or climbing.'));
    }
  }
  ...
  const html = `<div style="font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;color:#111827;max-width:720px;">`
    ...
    + (attentionRows.length > 0
      ? sectionHeading(`Lifts to watch — trend over the last ${prog.windowDays} days`) + table(attentionRows)
      : '')
