...
// How many weeks of eating-out history the meals section charts, the reported
// week included.
const ATE_OUT_WEEKS = 4;
...
function proteinByDay(log, days) {
  return days.map(key => {
    const macros = macrosForDay(log[key]);
    return {
      date: key,
 ...
 * reaching back a month costs nothing extra. A week with no log at all still
 * gets a row (zero), because "we ate in every night" and "we tracked nothing"
 * both look like an empty week from here and the run of dates is what makes
 * the trend readable.
 */
export function summarizeAteOutHistory(dailyLog, week, weeks = ATE_OUT_WEEKS) {
  ...
  for (let i = 0; i < weeks; i++) {
    ...
    // "Aug 9–15" when the week sits inside one month, "Aug 30–Sep 5" when it
    // straddles two. Four rows of these are read as a column, and repeating
    // the month on both ends of every one of them is noise.
...
export function summarizeGoals(data, week, config = {}) {
  ...
  const vegTarget = numOr(nutrition.vegServings, 5) * week.days.length;
  const fruitTarget = numOr(nutrition.fruitServings, 4) * week.days.length;
  ...
  add('🥦', 'Veg', Math.round(meals.veg * 10) / 10, Math.round(vegTarget * 10) / 10, ' servings');
  add('🍎', 'Fruit', Math.round(meals.fruit * 10) / 10, Math.round(fruitTarget * 10) / 10, ' servings');
...
export function summarizeWeek(data, week, { withProgress = false, goalsConfig = null } = {}) {
  return {
    week,
    // The eating-out history reaches back past this week's window, so it's
 ...
 */
function proteinChart(byDay, goal) {
  const days = Array.isArray(byDay) ? byDay : [];
  if (days.length === 0) return '';
  const values = days.map(d => (Number.isFinite(d.protein) ? d.protein : null));
  ...
  const caption = hasGoal
    ? `Protein per day · g <span style="color:#9ca3af;">— dashed line is your ${Math.round(goal)}g daily goal</span>`
    : `Protein per day · g`;
...
/**
 * Meals eaten out, one row per week, oldest at the top. Horizontal bars
 * because the label is a date range: four of those across the top of a column
 * chart is unreadable on a phone, four down the side is not.
 */
function ateOutChart(history) {
  const weeks = Array.isArray(history) ? history : [];
  if (weeks.length === 0) return '';
  const max = Math.max(...weeks.map(w => w.ateOut), 1);
  const rows = weeks.map((w, i) => {
    const last = i === weeks.length - 1;
    const pct = Math.round((w.ateOut / max) * 100);
    const color = last ? CHART_BAR : '#e0b3a3';
    const bar = `<table role="presentation" cellspacing="0" cellpadding="0" border="0" style="border-collapse:separate;width:100%;background:${CHART_TRACK};border-radius:999px;">`
      + `<tr>`
      + (pct > 0 ? `<td style="height:9px;line-height:9px;font-size:0;width:${pct}%;background:${color};border-radius:999px;">&nbsp;</td>` : '')
      + (pct < 100 ? `<td style="height:9px;line-height:9px;font-size:0;">&nbsp;</td>` : '')
      + `</tr></table>`;
    return `<tr>`
      + `<td style="padding:3px 10px 3px 0;font-size:12px;color:${last ? '#111827' : '#6b7280'};white-space:nowrap;${last ? 'font-weight:600;' : ''}">`
      + `${escapeHtml(w.label)}${last ? ' <span style="color:#9ca3af;font-weight:400;">this week</span>' : ''}</td>`
      + `<td style="padding:3px 0;width:50%;">${bar}</td>`
      + `<td style="padding:3px 0 3px 10px;font-size:12px;color:#111827;white-space:nowrap;font-weight:600;">${w.ateOut}</td>`
      + `</tr>`;
  });
  return `<div style="margin:14px 0 4px 0;font-size:12px;color:#6b7280;">Meals eaten out · last ${weeks.length} weeks</div>`
    + `<table role="presentation" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;width:100%;">${rows.join('')}</table>`;
}
 ...
 */
export function renderWeeklySummary({ stats, priorStats, goals = null, bodyStats = null, name = '' }) {
  const s = stats;
  const p = priorStats;
  const g = goals || {};
  ...
  const html = `<div style="font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;color:#111827;max-width:720px;">`
    ...
    + proteinChart(s.meals.proteinByDay, g.protein)
  ...
  if (proteinDays.length > 0) {
    const pGoal = Number(g.protein);
    const pMax = Math.max(...proteinDays.map(d => d.protein), pGoal > 0 ? pGoal : 0);
    t.push('');
    ...
    for (const d of s.meals.proteinByDay || []) {
      ...
      const bars = pMax > 0 ? Math.round((d.protein / pMax) * 12) : 0;
      t.push(`    ${d.dow}  ${'█'.repeat(bars)}${'░'.repeat(12 - bars)}  ${d.protein}`
        + (pGoal > 0 && d.protein >= pGoal ? '  ✓' : ''));
    }
  }
  const ateOutWeeks = s.meals.ateOutHistory || [];
  if (ateOutWeeks.length > 0) {
    const aMax = Math.max(...ateOutWeeks.map(w => w.ateOut), 1);
    const labelWidth = Math.max(...ateOutWeeks.map(w => w.label.length));
    t.push('');
    t.push(`  Meals eaten out, last ${ateOutWeeks.length} weeks:`);
    for (const w of ateOutWeeks) {
      const bars = Math.round((w.ateOut / aMax) * 12);
      t.push(`    ${w.label.padEnd(labelWidth)}  ${'█'.repeat(bars)}${'░'.repeat(12 - bars)}  ${w.ateOut}`);
    }
  }
  t.push('');
  t.push('WEIGHT');
