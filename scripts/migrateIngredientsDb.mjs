#!/usr/bin/env node
/**
 * Move `ingredientsDb` off the user document into users/{uid}/data/ingredients.
 *
 * The ingredient DB is a single ~270KB array that every edit rewrites whole,
 * sitting on the same document as habits, restaurants and the rest — and on the
 * admin's account it's the reason every signed-in client reads that whole
 * document. This copies it into its own doc, mirroring users/{uid}/data/recipes.
 *
 * Two phases, deliberately separate:
 *   --copy    write users/{uid}/data/ingredients from the user-doc field.
 ...
 *
 * Usage:
 *   node scripts/migrateIngredientsDb.mjs --copy    --key=<serviceAccount.json> [--email=<one user>]
 *   node scripts/migrateIngredientsDb.mjs --cleanup --key=<serviceAccount.json> [--email=<one user>]
...
for (const uid of await targetUids()) {
  ...
  const subRef = db.doc(`users/${uid}/data/ingredients`);
  const [userSnap, subSnap] = await Promise.all([userRef.get(), subRef.get()]);
  const field = userSnap.exists ? userSnap.data().ingredientsDb : undefined;
  const sub = subSnap.exists ? subSnap.data().ingredients : undefined;
  ...
  if (args.copy) {
    ...
    if (!dry) await subRef.set({ ingredients: fieldRows, migratedAt: new Date().toISOString() });
    copied++;
  }
  ...
  if (args.cleanup) {
    ...
    if (!dry) await userRef.update({ ingredientsDb: FieldValue.delete() });
    cleaned++;
  }
}
