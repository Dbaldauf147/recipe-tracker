...
/**
 * Expand size-based ingredients into small/medium/large variants.
 * For each ingredient that has SIZE_GRAMS data, creates 3 entries
 * with scaled nutrition. Skips if variants already exist.
 */
const SIZE_DATA = {
  'egg':              { small: 38, medium: 44, large: 50 },
  ...
  'pineapple':        { small: 450, medium: 680, large: 900 },
  'grapefruit':       { small: 200, medium: 246, large: 300 },
  'kiwi':             { small: 60, medium: 76, large: 90 },
  'plum':             { small: 55, medium: 66, large: 90 },
  'nectarine':        { small: 130, medium: 142, large: 175 },
  'apricot':          { small: 30, medium: 35, large: 45 },
  'beet':             { small: 80, medium: 115, large: 170 },
  'turnip':           { small: 90, medium: 130, large: 200 },
  'parsnip':          { small: 80, medium: 120, large: 170 },
};

const NUTRITION_KEYS = ['protein', 'carbs', 'fat', 'sugar', 'sodium', 'potassium', 'vitaminB12', 'vitaminC', 'magnesium', 'fiber', 'zinc', 'iron', 'calcium', 'calories', 'addedSugar', 'saturatedFat', 'leucine', 'omega3'];

export function expandSizeVariants(ingredients) {
  const existing = new Set(ingredients.map(i => (i.ingredient || '').toLowerCase().trim()));
  const toAdd = [];
  ...
  for (const [baseName, sizes] of Object.entries(SIZE_DATA)) {
    ...
    for (const [size, grams] of Object.entries(sizes)) {
      const variantName = `${baseItem.ingredient} (${size})`;
      if (existing.has(variantName.toLowerCase())) continue;

      const factor = grams / baseGrams;
      const variant = { ...baseItem, ingredient: variantName, grams: String(grams), measurement: size };

      for (const key of NUTRITION_KEYS) {
        const val = parseFloat(baseItem[key]);
        if (!isNaN(val) && val > 0) {
          variant[key] = String(Math.round(val * factor * 100) / 100);
        }
      }

      // Recompute proteinPerCal and fiberPerCal
      const cal = parseFloat(variant.calories) || 0;
      const prot = parseFloat(variant.protein) || 0;
      const fib = parseFloat(variant.fiber) || 0;
      if (cal > 0) {
        variant.proteinPerCal = String(Math.round((prot / cal) * 1000) / 10);
        variant.fiberPerCal = String(Math.round((fib / cal) * 1000) / 10);
      }

      toAdd.push(variant);
      existing.add(variantName.toLowerCase());
    }
  }

  if (toAdd.length === 0) return ingredients;
  console.log(`Expanded ${toAdd.length} size variants for ingredients`);
  return [...ingredients, ...toAdd].sort((a, b) => (a.ingredient || '').localeCompare(b.ingredient || ''));
}
