...
// The nutrient catalogue lives in nutrients.js so pure logic can import it
// without pulling in this module's import.meta.env / fetch dependencies.
// Re-exported here because most callers import it from './nutrition'.
export { NUTRIENTS, NUTRIENT_BY_KEY, KCAL_PER_GRAM } from './nutrients.js';
...
async function fetchFromUSDA(ingredient, dataTypes = 'Foundation,SR%20Legacy') {
  ...
  nutrients.fermentedServings = computeFermentedServings(name, grams);
  nutrients.omega3Servings = computeOmega3Servings(name, grams);
...
async function fetchFromOpenFoodFacts(ingredient) {
  ...
  const nutrients = {
    ...
    fermentedServings: computeFermentedServings(name, grams),
    omega3Servings: computeOmega3Servings(name, grams),
  };
